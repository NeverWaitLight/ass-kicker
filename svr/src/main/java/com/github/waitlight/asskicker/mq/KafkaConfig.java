...
public class KafkaConfig {
    ...
    @Value("${spring.kafka.consumer.group-id:ass-kicker-worker}")
    private String consumerGroupId;
    @Value("${ass-kicker.kafka.send-tasks-topic-partitions:3}")
    private int sendTasksTopicPartitions;

    @Bean
    public NewTopic sendTasksTopic() {
        return TopicBuilder.name(SEND_TASKS_TOPIC)
                .partitions(sendTasksTopicPartitions)
                .replicas(1)
                .build();
    }
